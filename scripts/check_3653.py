...
d=json.load(open('results/3653/extracted.json'));a=np.array(Image.open('input/IMG_3653.JPG')).astype(float);cols=np.array(d['colors']);diffs=[]
for p in d['parts']:
 for r,row in enumerate(p['grid']):
  for c,v in enumerate(row):
   if v<0:continue
   x=round(p['source'][0]+(c+.5)*15.65);y=round(p['source'][1]+(r+.5)*15.65)
   corners=np.concatenate([a[y+dy-1:y+dy+2,x+dx-1:x+dx+2].reshape(-1,3) for dy in (-4,4) for dx in (-4,4)])
...
if diffs:
 ...
 im.save('results/3653/color-check.png')
