...
d=json.load(open('results/3654/extracted.json'));a=np.array(Image.open('input/IMG_3654.JPG')).astype(float);cols=np.array(d['colors']);diffs=[]
for p in d['parts']:
 for r,row in enumerate(p['grid']):
  for c,v in enumerate(row):
   if v<0:continue
   x=round(p['source'][0]+(c+.5)*18.7);y=round(p['source'][1]+(r+.5)*18.7)
...
if diffs:
 ...
 im.save('results/3654/color-check.png')
