...
d=json.load(open('results/3650/extracted.json'));a=np.array(Image.open('input/IMG_3650.JPG')).astype(float);cols=np.array(d['colors']);diffs=[]
for p in d['parts']:
 for r,row in enumerate(p['grid']):
  for c,v in enumerate(row):
   if v<0:continue
   x=round(p['source'][0]+(c+.5)*25.25);y=round(p['source'][1]+(r+.5)*25.25)
   corners=np.concatenate([a[y+dy-1:y+dy+2,x+dx-1:x+dx+2].reshape(-1,3) for dy in (-8,8) for dx in (-8,8)])
...
print('Corner sample differences',diffs)
if diffs:
 ...
 im.save('results/3650/color-check.png')
