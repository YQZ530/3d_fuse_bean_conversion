...
# MARD E22. Keep this local so the renderer does not depend on a palette file
# from a different repository.
background = '#B785A1'

source = Path('scripts/render_3652.py').read_text(encoding='utf-8')
...
source = source.replace('wheatfield-cypresses-52x{ROWS}', 'wheatfield-cypresses-52x{ROWS}-e22')
...
exec(compile(source, 'scripts/render_3652.py [E22 background]', 'exec'))

report_path = Path('results/3652/verification-e22.json')
report = json.loads(report_path.read_text(encoding='utf-8'))
report['background'] = {'code': 'E22', 'hex': background, 'beads': False}
original = json.loads(Path('results/3652/verification.json').read_text(encoding='utf-8'))
assert all(report[key] == original[key] for key in ('columns', 'rows', 'parts', 'beads', 'counts'))
assert Path('results/3652/wheatfield-cypresses-52x90-e22.csv').read_bytes() == Path('results/3652/wheatfield-cypresses-52x90.csv').read_bytes()
